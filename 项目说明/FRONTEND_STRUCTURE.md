...
│   └── store/              # 状态管理 (Zustand)
│       ├── index.ts        # Store 统一导出
│       │   ├── 所有 Store 的导出         # 中央化状态管理导出
│       │   ├── Store 类型定义           # Store 相关类型
│       │   └── Store 组合器             # 多 Store 组合逻辑
...
│       ├── MessageList.tsx      ✅ 消息列表，滚动控制，虚拟滚动
...
├── ui/                     📋 计划中
│   ├── dialog.tsx          🔄 下个阶段
│   ├── table.tsx           🔥 高优先级 (数据展示需要)
│   ├── badge.tsx           📋 待规划
│   └── ...                 📋 根据需求添加
├── data/                   🔥 当前优先级
│   ├── TableDisplay.tsx    🔄 计划开发 (查询结果展示)
│   ├── SchemaView.tsx      🔄 计划开发 (数据库结构)
│   ├── PreviewCard.tsx     📋 待规划 (操作预览)
...
**文档版本**: v3.0  
**最后更新**: 2025-01-31 23:10  
**实现进度**: 核心聊天功能完成，数据展示组件开发中 