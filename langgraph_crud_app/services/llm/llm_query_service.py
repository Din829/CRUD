...
5.  **复合操作 (composite)**: 指在一个用户请求中，明确组合了 (a) 新增、修改、删除中的至少两种不同操作类型，或 (b) 针对多个独立主数据实体（如不同表、或同一表内完全分离的记录集）的同类或不同类操作，或 (c) 存在明确顺序依赖的操作序列（如"新增订单X，然后添加订单项Y"）。**重要**: 单一复杂操作（如新增一条含多个字段的记录，或删除满足某条件的多条记录）不视为复合，由对应专项流程处理。示例: "修改用户A邮箱(改)并新增用户B地址(增)"；"删除users表用户X(删)并删除orders表相关订单(删)"。
