...
4.  **删除 (delete)**: 删除记录，含关键词：删除、移除、取消。示例："删除 TKT-2307-0001"；"找出所有过期的任务并删除它们"
5.  **复合操作 (composite)**: 
    指在一个用户请求中，明确包含以下情况之一：
    (a) 针对不同数据实体的多个独立写操作（新增、修改、删除），例如："修改用户A的邮箱并为用户B新增一条地址记录"；"删除users表的用户X并同时删除orders表中的相关订单"。
    (b) 对同一数据实体执行多种不同类型的写操作序列，例如："新增产品P，然后立即更新其库存"。
    (c) 存在明确的、由用户指定的、跨多个独立操作的顺序依赖，例如："先执行A操作，完成后再执行B操作"。
    **重要**: 以下情况**不**视为复合操作，应由对应的专项流程（如 `delete`, `modify`, `add`）处理：
      1. 单一的写操作（新增、修改、删除），即使其条件复杂或影响多条记录。例如，"删除所有状态为'过期'且创建时间早于去年的任务"应归为 `delete`。
      2. 当用户指令中包含的"查找"、"找出"等查询性描述，其目的仅是为了定义后续单一写操作（新增、修改、删除）的目标范围或条件时。例如："查找所有状态为'pending'的OCR任务并删除它们"应归为 `delete`；"找出用户'张三'并更新他的电话号码"应归为 `modify`。
6.  **确认/其他 (confirm_other)**: 含关键词：保存、确认、是、好、确定，或无法归类到以上任何一种。示例："保存""是""继续"
7.  **重置 (reset)**: 重置或清空，含关键词：重置、重新开始、清空。示例："重置所有数据"

规则:
- 如果用户的查询意图主要是获取信息或数据分析报告（例如，用户希望看到查询结果或统计数据），则优先归类为 **查询/分析 (query_analysis)**。
- 如果用户指令明确包含多种独立的数据库写操作（新增、修改、删除中的两种或以上，且这些操作针对不同目标或形成一个必须按顺序执行的序列），则归类为 **复合操作 (composite)**。
- 如果用户指令可以被理解为对单一目标数据集执行单一主要写操作（新增、修改、删除），即使该操作的目标或条件需要通过用户描述中的查找条件来确定（例如，"查找X并删除X"，"找出Y并更新Y"），也应优先归类到对应的单一操作意图（`add`, `modify`, `delete`）。
...
规则:
1.  **查询 (query)**:
    - 用户意图是检索、获取或列出一条或多条记录的具体信息。这可以包括记录的全部字段或用户指定的特定字段。
    - 通常涉及根据特定条件筛选数据。
    - 包含关键词：查询、搜索、查找、查、列出、显示、获取、详情、状态、内容等。
    - 示例："查询 TKT-2307-0001 的状态"，"搜索 TKT-2307-0001 的完整信息"，"列出所有2024年创建的工单及其负责人和创建日期"，"查询用户 John Doe 的邮箱和电话号码"。
2.  **分析 (analysis)**:
    - 用户意图是对数据进行统计、汇总、聚合或计算衍生指标，以发现趋势、模式或总结性信息。
    - 通常涉及聚合函数（如 COUNT, SUM, AVG, MAX, MIN）或 GROUP BY 子句。
    - 不以直接列出原始记录的详细字段为主要目的，而是生成概括性数据。
    - 包含关键词：统计、分析、汇总、多少、总数、分布、平均、趋势、比例、占比、最等。
    - 示例："数据表中一共多少工单?"，"统计每个部门的工单数量"，"分析不同优先级工单的平均解决时间"，"找出提示数量最多的前三个用户"。
...
重要规则：
1.  **表和字段**: 优先使用提供的表结构中的表和字段。如果用户明确提到不在可用表列表中的表名，仍然按用户意图生成SQL，让数据库处理错误。
...
6.  **模糊或无效查询**: 如果用户输入过于模糊、信息不足以定位具体表/字段，固定返回字符串：\"ERROR: 请澄清你的查询条件，例如提供完整编号或指定具体字段。\"
